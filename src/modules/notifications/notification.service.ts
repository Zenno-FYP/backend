...
export class NotificationService {
  ...
  /**
   * Send a one-off test push to every device registered for [userId].
   *
   * Bypasses the per-type preference flags (chat / project / digest) so
   * the user can verify that FCM delivery is wired end-to-end even if
   * they happen to have all the categories disabled in settings. Still
   * respects `push_enabled` though — if the user globally turned off
   * push, we report `pushed: false` so the UI can tell them.
   *
   * Returns a structured result the mobile app can show in a snackbar.
   */
  async sendTestNotification(userId: Types.ObjectId): Promise<{
    pushed: boolean;
    deviceCount: number;
    successCount: number;
    reason?: string;
    ...
    // Persist a row so the test push also lands in the in-app
    // notifications list — then the user can verify both surfaces from
    // a single tap of the test button.
    const notif = await this.notifModel.create({
      user_id: userId,
      type: 'test',
    ...
    if (result.failedTokens.length) {
      ...
      this.logger.log(
        `Test push purged ${result.failedTokens.length} stale FCM tokens`,
      );
    }
